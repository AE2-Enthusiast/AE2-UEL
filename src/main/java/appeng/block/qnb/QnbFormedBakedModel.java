...
class QnbFormedBakedModel implements IBakedModel {
	...
	if (false) {
	    if (layer == BlockRenderLayer.CUTOUT) {
		return this.getQuads(formedState, state, side, rand, layer);
	    } else {
		return Collections.emptyList();
	    }
	    } else {
    ...
    private List<BakedQuad> getQuads(QnbFormedState formedState, IBlockState state, EnumFacing side, long rand, BlockRenderLayer layer) {
	...
	if (layer == BlockRenderLayer.CUTOUT) {
	    ...
	    if (formedState.isPowered() && state.getBlock() != this.linkBlock) {
		if (formedState.isCorner()) {
			...
			builder.setDrawFaces(EnumSet.of(facing));
			builder.addCube(-xOffset, -yOffset, -zOffset, 16 + xOffset, 16 + yOffset,
					16 + zOffset);
		    }
		}
	    }
