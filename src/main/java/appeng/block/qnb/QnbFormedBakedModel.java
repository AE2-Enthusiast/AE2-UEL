			...
			if (state.getBlock() == this.linkBlock) {
				...
				if (formedState.isCorner()) {
					...
					if (formedState.isPowered()) {
						CubeBuilder bloomBuilder = new CubeBuilder(this.vertexFormat, this.bloomQuads);
						bloomBuilder.setTexture(this.lightCornerTexture);
						...
						for (EnumFacing facing : EnumFacing.values()) {
							...
							bloomBuilder.setDrawFaces(EnumSet.of(facing));
							bloomBuilder.addCube(DEFAULT_RENDER_MIN - xOffset, DEFAULT_RENDER_MIN - yOffset,
					...
					builder.addCube(0, DEFAULT_RENDER_MIN, DEFAULT_RENDER_MIN, 16, DEFAULT_RENDER_MAX,
							DEFAULT_RENDER_MAX);

					builder.addCube(DEFAULT_RENDER_MIN, 0, DEFAULT_RENDER_MIN, DEFAULT_RENDER_MAX, 16,
							DEFAULT_RENDER_MAX);

					builder.addCube(DEFAULT_RENDER_MIN, DEFAULT_RENDER_MIN, 0, DEFAULT_RENDER_MAX, DEFAULT_RENDER_MAX,
							16);

					if (formedState.isPowered()) {
						CubeBuilder bloomBuilder = new CubeBuilder(this.vertexFormat, this.bloomQuads);
						bloomBuilder.setTexture(this.lightTexture);
						bloomBuilder.setRenderFullBright(true);
						for (EnumFacing facing : EnumFacing.values()) {
							// Offset the face by a slight amount so that it is drawn over the already drawn
							// ring texture
							...
							bloomBuilder.setDrawFaces(EnumSet.of(facing));
							bloomBuilder.addCube(-xOffset, -yOffset, -zOffset, 16 + xOffset, 16 + yOffset,
									16 + zOffset);
						}
					}
				}
